...
raw_ose_count_yield_correlation <- dat_just_s3 |>
  ggplot(aes(x=ose_num_adj,y=yield)) +
    geom_point() +
    theme_pubr() +
    xlab("locust count")

raw_ose_damage_yield_correlation <- dat_wo_m1 |>
  ggplot(aes(x=att_mil_adj,y=yield)) +
    geom_point() +
    theme_pubr() +
    xlab('locust damage')
...
ose_count_damage_correlation <- dat |>
  ggplot(aes(x=ose_num,y=att_mil_adj,color=treatment)) +
    geom_point() +
    theme_pubr() +
...
yield_locust_mod0 <- gam(yield ~ 1,data=dat_wo_m1,family=tw())

yield_locust_mod1 <- gam(yield ~ ose_num_adj,data=dat_wo_m1,family=tw())

# yield_locust_mod2 <- gam(yield ~ ose_num_adj +
#                              s(mission,bs='re'),
#                          data=dat_wo_m1,family=tw())

summary(yield_locust_mod1)
draw(yield_locust_mod1,parametric = TRUE)
