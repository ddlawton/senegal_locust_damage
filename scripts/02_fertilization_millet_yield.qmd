...
dat_wo_s1 <- dat |> filter(survey != 's1')


dat_s3 <- dat |> 
    filter(survey %in% c('s3','s4')) |>
    group_by(village,farmer_num,treatment) |>
    fill(yield,.direction='updown') |>
    filter(survey == 's4')
...
raw_yield_farmer_plot <- dat_wo_s1 |>
  select(farmer_num,treatment,yield,village) |>
  distinct() |>
...
raw_yield_plot <- dat_wo_s1 |>
  select(farmer_num,treatment,yield) |>
  distinct() |>
...
raw_yield_boxplot_plot <- dat_wo_s1 |>
  select(farmer_num,treatment,yield) |>
  distinct() |>
  ggplot(aes(x=treatment,y=yield)) +
...
ggsave(raw_yield_farmer_plot,
       file='output/raw_yield_plots/raw_yield_farmer_plot.pdf',
       width=10,height=5,dpi=300)

ggsave(raw_yield_plot,
       file='output/raw_yield_plots/raw_yield_plot.pdf',
       width=5,height=5,dpi=300)

ggsave(raw_yield_boxplot_plot,
       file='output/raw_yield_plots/raw_yield_boxplot_plot.pdf',
       width=5,height=5,dpi=300)
...
ggsave(yield_anova_glmms_estimated_marginal_means_comparisons,
       file='output/modeled_yield_plots/yield_anova_glmms_estimated_marginal_means_comparisons.pdf',
       width=5,height=5,dpi=300)
```
...
ggsave(final_yield_model_est_marginal_means,
       file='output/modeled_yield_plots/final_yield_model_est_marginal_means.pdf',
       width=5,height=5,dpi=300)

ggsave(farmer_yield_variance,
       file='output/modeled_yield_plots/farmer_yield_variance.pdf',
...
param_table <- mod3 |> broom::tidy(parametric = TRUE)
smooth_table <- mod3 |> 
...
write_csv(combined_table,file=here('output/model_summary_files/yield_fertilizer_model_results.csv'))
...
yield_data <- dat |>
  select(farmer_num,village,treatment,yield,survey) |>
