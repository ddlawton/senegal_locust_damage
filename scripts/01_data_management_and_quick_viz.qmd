...
dat <- read_xlsx(here('data/alldata_v19.xlsx'),sheet = 1,.name_repair = 'minimal') |>
  select(1:28) |>
  clean_names() 
...
# Using pivot_longer
dat4 <- dat %>%
  pivot_longer(cols = starts_with(c('s1_', 's2_', 's3_','s4_')), 
...
dat5 <- dat4 |>
  ...
  select(farmer_num,village,treatment,survey:yield,datetime) |>
...
dat6 |>
  ggplot(aes(x=ose_num_adj,y=att_mil)) +
    geom_point() +
...
dat6 |>
  ggplot(aes(y=ose_num_adj,x=mission,color=village)) +
    geom_jitter(width=0.3,height=0) +
    MetBrewer::scale_color_met_d(name = 'Degas') +
    theme_pubr()

dat6 |>
  ggplot(aes(y=att_mil_adj,x=mission,color=village)) +
    geom_jitter(width=0.3,height=0)  +
    MetBrewer::scale_color_met_d(name = 'Degas') +
    theme_pubr()
...
coordinates <- st_as_sf((dat6 %>% drop_na(longitude, latitude)), coords = c("longitude", "latitude"), crs = 4326)
...
ggplot() +
  geom_sf(data = africa) +
  geom_sf(data = coordinates,aes(color=att_mil_adj)) +
  theme_void() +
  facet_wrap(~mission,ncol = 2) +
  scale_color_viridis_c(trans = 'sqrt')
