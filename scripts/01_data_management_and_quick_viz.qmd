...
plots <- list(
  "scatter_treatment_ose_num" = dat5 |>
    ggplot(aes(x = treatment, y = ose_num)) +
    geom_jitter(width = 0.2, height = 0) +
    labs(x = "treatment", y = "locust Count") +
    theme_pubr(),
  
  "scatter_treatment_att_mil" = dat5 |>
    ggplot(aes(x = treatment, y = att_mil)) +
    geom_jitter(width = 0.2, height = 0) +
    labs(x = "treatment", y = "locust damage") +
    theme_pubr(),

  "scatter_treatment_yield" = dat5 |>
    ggplot(aes(x = treatment, y = yield)) +
    geom_jitter(width = 0.2, height = 0) +
    labs(x = "treatment", y = "yield") +
    theme_pubr(),

  "scatter_ose_num_att_mil" = dat5 |>
    ggplot(aes(x = ose_num, y = att_mil)) +
    geom_point() +
    geom_smooth(method = "lm") +
  ...
  "scatter_ose_num_temp" = dat5 |>
    ggplot(aes(y = ose_num, x = temp_c)) +
    geom_point() +
    labs(x = "temperature (°C)", y = "locust Count") +
    theme_pubr(),

  "scatter_ose_num_humidity" = dat5 |>
    ggplot(aes(y = ose_num, x = humidity_percent)) +
    geom_point() +
    labs(x = "humidity (%)", y = "locust Count") +
    theme_pubr(),

  "scatter_ose_num_ground_cover" = dat5 |>
    ggplot(aes(y = ose_num, x = ground_cover_percent)) +
    geom_point() +
    labs(x = "ground cover (%)", y = "locust count") +
    theme_pubr(),

  "scatter_ose_num_mission_village" = dat5 |>
    ggplot(aes(y = ose_num, x = mission, color = village)) +
    geom_jitter(width = 0.3, height = 0) +
    MetBrewer::scale_color_met_d(name = "Degas") +
    labs(x = "mission", y = "locust Count", color = "village") +
    theme_pubr(),

  "scatter_att_mil_mission_village" = dat5 |>
    ggplot(aes(y = att_mil, x = mission, color = village)) +
    geom_jitter(width = 0.3, height = 0) +
    MetBrewer::scale_color_met_d(name = "Degas") +
    labs(x = "mission", y = "locust damage", color = "village") +
    theme_pubr()
)
...
plots[["spatial_ose_num"]] <- ggplot() +
  geom_sf(data = africa) +
  geom_sf(data = coordinates, aes(color = ose_num)) +
  theme_void() +
  facet_wrap(~mission, ncol = 2) +
  scale_color_viridis_c(trans = "sqrt") +
  labs(color = "locust Count")

plots[["spatial_att_mil"]] <- ggplot() +
  geom_sf(data = africa) +
  geom_sf(data = coordinates, aes(color = att_mil)) +
  theme_void() +
  facet_wrap(~mission, ncol = 2) +
  scale_color_viridis_c(trans = "sqrt") +
  labs(color = "locust damage")
