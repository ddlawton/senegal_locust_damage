...
plot_list <- list(
  raw_ose_count_treatment = dat |>
    select(village,farmer_num,treatment,ose_num) |>
    distinct() |>
  ...
  raw_ose_count_humidity_percent = dat |>
    select(village,farmer_num,treatment,ose_num,humidity_percent) |>
    distinct() |>
  ...
  raw_ose_count_ground_cover_percent = dat |>
    select(village,farmer_num,treatment,ose_num,ground_cover_percent) |>
    distinct() |>
  ...
  raw_ose_count_hour = dat |>
    select(village,farmer_num,treatment,ose_num,hour) |>
    distinct() |>
...
ose_number_village_survey <- dat |>
  filter(survey != 's1') |>
  select(village,survey,farmer_num,treatment,ose_num) |>
  distinct() |>
  drop_na() |>
...
ose_number_treatment_village_survey <- dat |>
  ...
  distinct() |>
  drop_na() |>
  ggplot(aes(x=survey,y=ose_num,color=village,group=farmer_num)) +
    geom_line(alpha=0.5) +
    geom_point(size=2) +
    scale_color_met_d(name='Demuth') +
    ylab(bquote("locust count" ~ (individuals %*% 100 ~ m^2))) +
...
locust_dat <- dat |>
  select(farmer_num,village,treatment,ose_num,att_mil,survey) |>
