...
plot_list <- list(
  ...
  raw_millet_attack_treatment = dat |>
    select(village,farmer_num,treatment,att_mil) |>
    distinct() |>
  ...
  raw_millet_attack_humidity_percent = dat |>
    select(village,farmer_num,treatment,humidity_percent,att_mil) |>
    distinct() |>
  ...
  raw_millet_attack_ground_cover_percent = dat |>
    select(village,farmer_num,treatment,ground_cover_percent,att_mil) |>
    distinct() |>
  ...
  raw_millet_attack_hour = dat  |>
    select(village,farmer_num,treatment,hour,att_mil) |>
    distinct() |>
...
dat |>
  filter(survey != 's4') |>
  ggplot(aes(x=survey,y=att_mil,color=village,group=farmer_num)) +
    geom_line(alpha=0.5) +
    geom_point(size=2) +
    scale_color_met_d(name='Demuth') +
    theme_pubr() +
...
locust_dat <- dat |>
  select(farmer_num,village,treatment,ose_num_adj,att_mil,survey) |>
